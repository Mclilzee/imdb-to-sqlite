...
async fn main() -> Result<(), String> {
    ...
    if Path::new(&args.path).exists() || args.overwrite {
        File::create(&args.path).map_err(|e| format!("Failed to create file {} => {e}", args.path))?;
    }
    ...
    if let Err(str) =
        name_titles::parse_name_titles(NAME_BASICS_FILE, NAME_TITLE_TABLE, &mut conn).await
    {
        eprintln!("\n{str}");
    }

    if let Err(str) =
        title_genres::parse_title_genres(TITLE_BASICS_FILE, TITLE_GENRES_TABLE, &mut conn).await
    {
        eprintln!("\n{str}");
    }

    if let Err(str) =
        title_ratings::parse_title_ratings(TITLE_RATING_FILE, TITLE_RATING_TABLE, &mut conn).await
    {
        eprintln!("\n{str}");
    }

    if let Err(str) =
        name_professions::parse_name_professions(NAME_BASICS_FILE, NAME_PROFESSION_TABLE, &mut conn)
            .await
    {
        eprintln!("\n{str}");
    }
    if let Err(str) = title_principals::parse_title_principals(
    ...
    if let Err(str) =
        title_directors::parse_title_directors(TITLE_CREW_FILE, TITLE_DIRECTORS_TABLE, &mut conn)
            .await
    {
        eprintln!("\n{str}");
    }
    if let Err(str) =
        title_writers::parse_title_writers(TITLE_CREW_FILE, TITLE_WRITERS_TABLE, &mut conn).await
    {
        eprintln!("\n{str}");
    }
    if let Err(str) =
        title_episodes::parse_title_episodes(TITLE_EPISODE_FILE, TITLE_EPISODE_TABLE, &mut conn)
            .await
    {
        eprintln!("\n{str}");
    }
